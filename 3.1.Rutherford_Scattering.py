...
clear_dead_particles_button = button(
    bind=clear_dead_particles, text="Clear Dead Particles"
)
...
def Run(b):  # b = button
    ...
    running = not running
    if running:
        b.text = "Pause Movement"
        dt = remember_dt
    else:
        b.text = "Resume Movement"
        remember_dt = dt
        dt = 0
    return


button(text="Pause Movement", bind=Run)


running_ = True


def Spawn(bu):  # bu = button
    global running_
    running_ = not running_
    if running_:
        bu.text = "Pause Spawning"
    else:
        bu.text = "Resume Spawning"
    return


button(text="Pause Spawning", bind=Spawn)
...
for time in range(0, 1000, dt):
    ...
    if running_ and i % alpha_rate == 0:
